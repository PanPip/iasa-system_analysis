...
class Solve(object):
    ...
    def poly_func(self):
        ...
        :return: function
        '''
        if self.poly_type =='chebyshev1':
            self.poly_f = special.eval_sh_chebyt
        elif self.poly_type =='chebyshev2':
            self.poly_f = special.eval_sh_chebyu
    ...
    def built_F(self):
        ...
        for i in range(self.Y.shape[1]):
            ...
            #self.norm_error.append(np.linalg.norm(self.Y[:,i] - self.F[:,i]))
    ...
    def built_F_(self):
        ...
        for i in range(self.Y_.shape[1]):
            ...
            #self.error.append(np.linalg.norm(self.Y_[:,i] - self.F_[:,i]))
    ...
    def save_to_file(self):
        ...
        ws.append(['Y перестроенное нормализованное :'])
        for i in range(self.n):
             ws.append(l+self.F[i].tolist()[0])
        ws.append([])

        ws.append(['Y перестроенное :'])
        for i in range(self.n):
             ws.append(l+self.F_[i].tolist()[0])
        ws.append([])

        ws.append(['Нормализовная невязка (Y -Ф)'])
        ws.append(l + self.norm_error)

        ws.append(['невязка (Y_ - Ф_))'])
