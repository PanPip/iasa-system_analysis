...
class PolynomialBuilder(object):
    def __init__(self, solution):
        ...
        if solution.poly_type == 'chebyshev1':
            self.symbol = 'T'
            self.basis = b_gen.basis_sh_chebyshev1(max_degree)
        if solution.poly_type == 'chebyshev2':
            self.symbol = 'U'
            self.basis = b_gen.basis_sh_chebyshev2(max_degree)
    ...
    def get_results(self):
        ...
        f_strings = ['(F{0})={result}\n'.format(i + 1, result=self._print_F_i(i))
                     for i in range(self._solution.Y.shape[1])]
        f_strings_transformed = ['(Ф{0}) трансформированный:\n{result}\n'.format(i + 1, result=self._print_F_i_transformed(i))
                                 for i in range(self._solution.Y.shape[1])]
        f_strings_transformed_denormed = ['(Ф{0}) трансформированный ' \
                                          'денормированный:\n{result}\n'.format(i + 1, result=
    ...
    def plot_graphs(self):
        ...
        for index in range(self._solution.Y.shape[1]):
            ...
            norm_ax.plot(np.arange(1, self._solution.n + 1),
                         abs(self._solution.Y_[:, index] - self._solution.F_[:, index]), 'b-')
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
            norm_ax.text(1, 10, 'норм. невязка(max):'+str(self._solution.norm_error[index])+'\nнорм. невязка(avg)'+str(self._solution.norm_error_a[index]), fontsize=11, verticalalignment='top',bbox = props)
